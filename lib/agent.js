var exec    = require('child_process').exec,
	...
	http    = require('http'),
	async   = require('async'),
	config  = _.extend(require('../config/default.js'), require('../config/config.json')),
	mysql   = require('mysql'),
	fs 	    = require('fs'),
...
Agent.prototype.run = function (pollFreq) {
	...
	// Do the checks
    setInterval(_.bind(this.postChecks, this), this.pollFreq);
};

/**
 * runs the checks, collects the data and posts to the server.
 * @see this.doChecks()
 * @see this.post()
 */
Agent.prototype.postChecks = function () {
	
	this.doChecks( _.bind(function (err, results) {
        this.log('----------------------------------------');
...
/**
 * Checks
 * Run all check functions and collect the data into a json object passed 
 * to the callback function
 * @param function callback passed err, results properties. results (the second parameter)
 * contains the json object containing the results of each test.
 */
Agent.prototype.doChecks = function (callback) {
    ...
    async.parallel({
		...
		mongo			: _.bind(require('./mongo'), this),
...
Agent.prototype.post = function (data) {
	...
	this.log('post payload - ' + new Date());
	this.log(data);
...
Agent.prototype.getApacheStatus = function (callback) {
	...
	var processAacheStatus = _.bind(function (response) {
		...
		if (response.statusCode !== 200) {
            this.error('Unable to get Apache status', 'getApacheStatus', '', callback);
			return false;
        }
		
		response.on('error', _.bind(function (e) {
			this.error('Unable to get Apache status - HTTPError ', 'getApacheStatus', e, callback);
			return false;
		}, this));
    ...
    req.on('error', _.bind(function (err){
        this.error('ApacheStatus: unable to connect to the Apache status url http://127.0.0.1/server-status?auto, has this been configured? And is the apache server running?', 'getApacheStatus', err, callback);
        return false;
    }, this));
    req.end();
...
Agent.prototype.getMySqlStatus = function(callback){
    ...
    if (_.isUndefined(config.mysql.host) || _.isUndefined(config.mysql.user)) {
        this.error('getMySQLStatus: config not set', 'getMySQLStatus', '', callback);
        return false;
    }
    ...
    db.on('error', _.bind(function(err) {
        this.error('mysql connection error ' + err.code + ' ' + err, 'getMySQLStatus', err, callback); 
        return false;
    }, this));
    ...
    db.query('SHOW STATUS LIKE "Connections"', _.bind(function(err, rows, fields) {
        ...
        if (err) {
			this.error('mysql: Connections ' + err, 'getMySQLStatus', err);return;}
    ...
    db.query('SHOW STATUS LIKE "Open_files"', _.bind(function (err, rows, fields) {
        if (err) {this.error('mysql: Open_files ' + err, 'getMySQLStatus', 'getMySQLStatus', err);return;}
...
Agent.prototype.getCpuStats = function (callback) {
	...
	if (process.platform == 'linux') {
		...
		exec('mpstat -P ALL 1 1', _.bind(function (err, stats) {
			...
			if (err) {
				this.error('Error whilst executing command "mpstat -P ALL 1 1" you may have to install sysstat: apt-get install sysstat ', 'getCpuStats', err, callback);
				return false;
			}
...
Agent.prototype.getIoStats = function (callback) {
	...
	if (process.platform == 'linux') {
		...
		exec('iostat -d 1 2 -x -k', _.bind(function (err, stats) {
			if (err) {
				this.error('Error executing command "iostat -d 1 2 -x -k" the command may not exist have you installed systat? Try: apt-get install sysstat', 'getIoStats', err, callback)
				return false;
			}
...
Agent.prototype.getMemoryUsage = function (callback) {
    ...
    if (process.platform === 'linux') {
        ...
        exec('top -l 1', _.bind(function (error, top, stderr) {
            if (error) { 
				this.error('failed to run command: top -l 1', 'getMemoryUsage', error, callback);
				return false;
			}
            this.log('getMemoryUsage:attempting exec (sysctl vm.swapusage)');
            exec('sysctl vm.swapusage', _.bind(function (error, sysctl, stderr) {
                if (error) {
					this.error('failed to run command: sysctl vm.swapusage', 'getMemoryUsage', error, callback);
					return false;
				}
...
Agent.prototype.getNetworkTraffic = function (callback) {
	...
	if (this.networkTrafficStore === undefined)
    ...
    if (process.platform == 'linux') {
        ...
        fs.readFile('/proc/net/dev', 'utf8', _.bind(function (err, data) {
            if (err) {
                this.error('Error whilst opening /proc/net/dev for reading', 'getNetworkTraffic', err, callback);
                return false;
            }
		...
		exec('netstat -nbid', _.bind(function (err, netstat){
			...
			if (err) {
				this.error('Error executing command netstat -nbid', 'getNetworkTraffic', err, callback);
				return false;
			}
...
Agent.prototype.getProcesses = function (callback) {
    ...
    exec('ps auxww', _.bind(function (err, ps, stderr){
        if (err) {
            this.error('Command "ps auxww" failed', 'getProcesses', err, callback);
			return false;
        }
		...
		this.log('getProcesses: complete');
 ...
 * @param string msg error human readable message
 * @param string category a category string e.g. "connectingToMysql"
 * @param mixed error an error object
 * @param callback the async callback function to call in order to continue execution
 */
Agent.prototype.error = function (msg, category, error, callback) {
    this.errors.push({
