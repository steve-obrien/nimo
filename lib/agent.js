...
Agent.prototype.getIoStats = function (callback) {
	...
	if (process.platform == 'linux') {
		...
		exec('iostat -d 1 2 -x -k', _.bind(function (err, stats) {
			if (err) {
				this.error('Error executing command "iostat -d 1 2 -x -k" the command may not exist have you installed systat? Try: pt-get install sysstat', 'getIoStats', err, callback)
				return false;
			}
