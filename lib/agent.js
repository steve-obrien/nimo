...
Agent.prototype.run = function (pollFreq) {
    this.pollFreq = pollFreq || (config.nimon.pollFreq * 1000) || 60000;
...
Agent.prototype.runChecks = function () {
    ...
    _.bind(function(err, results) {
		...
		// post the results
		results.device = config.nimon.device || results.system.hostname
        this.post(results);
...
Agent.prototype.getCpuStats = function (callback) {
	...
	if (process.platform == 'linux') {
		...
		exec('mpstat -P ALL 1 1', function (err, stats) {
			
			if (err) {
				this.error('Errored whilst executing command "mpstat -P ALL 1 1"', 'getCpuStats', err);
				callback(null, {});
			}
			...
			for (i=4; i <= stats.length; i++) {
				...
				for (hi=0; hi <= headerNames.length-1; hi++) {
					...
					cpuStats[device][headerName] = values[hi];
				}					
			}
...
Agent.prototype.getIoStats = function (callback) {
	...
	if (process.platform == 'linux') {
		...
		exec('iostat -d 1 2 -x -k', function (err, stats) {
